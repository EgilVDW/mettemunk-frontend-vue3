...
export default defineNuxtComponent({
  async setup() {
    ...
    const { data, error } = await useAsyncData(
      `${currentRoute.value.fullPath}-page`,
      async () => {
        // await dictionaryStore.loadDictionaries();
        // await cmsPagesStore.loadPages();
        const cmsPagesStore = await cmsHelper.GetAllPages(config);
        ...
        // CONTACT
          ...
          const mapped: IContactPage = {
            ...
            departments: await Promise.all(
              cmsPage.departments.map(
                async (x) => await mapper.mapDepartment(x)
              )
            ),
        ...
        // PRODUCT
          ...
          throw new Error(
            `No page supports type '${currentPage._modelApiKey}'`
          );
        }
      }
    );
