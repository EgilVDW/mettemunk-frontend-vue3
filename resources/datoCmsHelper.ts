...
import type { ICmsBasicPage, ICmsDictionary } from "~/interfaces/cms/cmsBase";
import type {
  ICmsArticlePage,
...
export async function GetAllCategoryPages(): Promise<ICmsCategoryPage[]> {
  const result = await cmsClient.request(queries.getAllCategoryPages, {
    limit: 1,
  });
...
export async function GetProductPages(
  categoryId: string
): Promise<ICmsProductPage[]> {
  const result = await cmsClient.request(queries.products(categoryId), {
    limit: 1,
  });
...
export async function GetAllCategoryArticlePages(): Promise<ICmsArticlePage[]> {
  ...
  const result = await cmsClient.request(
    queries.GetAllCategoryArticlePages(listPage.id),
    { limit: 1 }
  );
...
export async function GetAllPages(): Promise<ICmsBasicPage[]> {
  ...
  const countResult = await cmsClient.request(routeQueries.getCountAllPages, {
    limit: 1,
  });
  ...
  for (
    let i = 0;
    i < Math.ceil(countResult._allArticlesMeta.count / 100);
    i++
  ) {
    const ArticlePageResult = await cmsClient.request(
      routeQueries.getAllArticlePages(i * 100),
      { limit: 1 }
    );
    // Articles
  ...
  for (
    let i = 0;
    i < Math.ceil(countResult._allLandingsMeta.count / 100);
    i++
  ) {
    const landingPageResult = await cmsClient.request(
      routeQueries.getAllLandingPages(i * 100),
      { limit: 1 }
    );
    // Landing pages
  ...
  for (
    let i = 0;
    i < Math.ceil(countResult._allCategoriesMeta.count / 100);
    i++
  ) {
    const categoriePageResult = await cmsClient.request(
      routeQueries.getAllCategoriePages(i * 100),
      { limit: 1 }
    );
    // Categories
  ...
  for (
    let i = 0;
    i < Math.ceil(countResult._allProductsMeta.count / 100);
    i++
  ) {
    const productPageResult = await cmsClient.request(
      routeQueries.getAllProductPages(i * 100),
      { limit: 1 }
    );
    // Products
  ...
  for (
    let i = 0;
    i < Math.ceil(countResult._allContactsMeta.count / 100);
    i++
  ) {
    const contactPageResult = await cmsClient.request(
      routeQueries.getAllContactPages(i * 100),
      { limit: 1 }
    );
    // Contacts
  ...
  const pageResult = await cmsClient.request(routeQueries.getAllSinglePages, {
    limit: 1,
  });
